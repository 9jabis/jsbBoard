	...
	<nav class="navbar navbar-default">
		...
		<div class="collapse navbar-collapse"
			...
			<ul class="nav navbar-nav">
				...
				<% if (boardID == 1){ %>
				<li class="active"><a href="bbs.jsp?boardID=1&pageNumber=1">식단
						평가</a></li>
				<li><a href="bbs.jsp?boardID=2&pageNumber=1">자유 게시판</a></li>
				<%} else if(boardID == 2){ %>
				<li><a href="bbs.jsp?boardID=1&pageNumber=1">식단 평가</a></li>
				<li class="active"><a href="bbs.jsp?boardID=2&pageNumber=1">자유
						게시판</a></li>
				<% } %>
			</ul>
			<%
			...
			<div class="container">
				<table class="table table-haver" style="border: 1px solid #dddddd">
					...
					<tr>
						...
						<%	JjimDAO jjimDAO = new JjimDAO();
							...
							if (list1.isEmpty()){%>
						<td align="right" bgcolor="beige"><button
								onclick="location.href='jjimAction.jsp?bbsID=<%=bbsID%>'">찜하기</button></td>
						<%	}
							else{%>
						<td align="right" bgcolor="beige"><button
								onclick="location.href='jjimAction.jsp?bbsID=<%=bbsID%>'">찜해제</button></td>
						<%	} %>
					</tr>
					<%} 
					if (boardID == 2){%>
				...
				%>
					<tr>
						<td colspan="6"><br>
						<br>
						<img src="bbsUpload/<%=bbsID %>사진.jpg" border="300px"
					...
					<tr align="center">
						<td colspan="2"><button
								onclick="location.href='evaluationAction.jsp?likeEat=1&sosoEat=0&badEat=0&bbsID=<%=bbsID%>'">
								좋아요(<%=like %>)
							</button></td>
						<td colspan="2"><button
								onclick="location.href='evaluationAction.jsp?likeEat=0&sosoEat=1&badEat=0&bbsID=<%=bbsID%>'">
								그럭저럭(<%=soso %>)
							</button></td>
						<td colspan="2"><button
								onclick="location.href='evaluationAction.jsp?likeEat=0&sosoEat=0&badEat=1&bbsID=<%=bbsID%>'">
								맛없어요(<%=bad %>)
							</button></td>
					</tr>
					<tr>
					...
					%>
						<td colspan="6" align="center"><a
							href="bbs.jsp?boardID=<%=boardID %>" class="btn-primary">목록</a>
							<%
						if(userID != null && userID.equals(bbs.getUserID())){
					%> <a href="update.jsp?bbsID=<%= bbsID %>&boardID=<%=boardID %>"
							class="btn-primary">수정</a> <a
							onclick="return confirm('정말로 삭제하시겠습니까?')"
							href="deleteAction.jsp?bbsID=<%= bbsID %>&boardID=<%=boardID %>"
							class="btn-primary">삭제</a></td>
					</tr>
					<% } %>
				</table>
			</div>
										...
										<table class="table table-striped"
											...
											<tbody>
												...
												%>
														<form name="p_search">
															<a type="button"
																onclick="nwindow(<%=boardID%>,<%=bbsID %>,<%=list.get(i).getCommentID()%>)"
											...
											%> <br>
													<br>
													<img
														src="commentUpload/<%=bbsID %>사진<%=list.get(i).getCommentID() %>.jpg"
						...
						<table class="table table-striped"
							...
							<tr>
								...
								<td><input type="text" style="height: 100px;"
									class="form-control" placeholder="좋은 글 부탁드립니다."
									name="commentText"></td>
								<td><br>
								<br>
